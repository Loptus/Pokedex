...
/**
 * Pages the list by offset, the same shape the API uses (`?limit=20&offset=40`).
 *
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, Pokemon> {
        ...
        return when (val result = loadPage(offset, params.loadSize)) {
            is AppResult.Success -> LoadResult.Page(
                ...
                // Advances by the page size asked for, not by how many entries survived parsing,
                // otherwise a dropped entry would make the next page repeat one.
